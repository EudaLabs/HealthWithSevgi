...
|-----------|------------|----------------|------|
| **React SPA** | React 18 + Vite + TypeScript + plain CSS (CSS variables) + Axios + TanStack React Query | Client-side UI — 7 step pages (Step 1–7), wizard progress tracker, 20-domain pill bar, interactive sliders, Recharts/Plotly charts, and data tables | `localhost:5173` |
...
| **Session Storage** | Backend in-memory LRU caches (no database, no browser localStorage) | Server-side session state — prepared datasets, trained models, SHAP artefacts. Evicted on process restart or LRU overflow; nothing persists across page reloads unless re-uploaded. | In-process with Backend |
